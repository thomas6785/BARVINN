...
void main_thread(const int hart_id){
    ...
    //int wbase_addr = 0;
    int iofst=0; // start input data at 0, why not
    int oofst=0x2000; // start the output at 0x2000 since a 32*32 input image (with 8 bits of precision) will take the first 8192 addresses up and we don't want to overwrite the input data
    int wofst=0;
    ...
    conv3x3_64(hart_id, iofst, oofst, wofst, 0, &iaddr, &oaddr, &waddr);
    iofst += *(&iaddr);
    oofst += *(&oaddr);
    wofst += *(&waddr);
...
int main(){
    int pito_hart_id = get_pito_hart_id();
    if (pito_hart_id == 0){
        printf("Starting convolution on Hart 0...\n");
        main_thread(0);
    } else {
        while(1);
    }
